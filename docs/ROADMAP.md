...
- [ ] `A0` — Remove `k_rocketarena` (world.c:819), `k_clan_arena`, `k_clan_arena_rounds`, `k_clan_arena_max_respawns` (world.c:823–825), `k_vp_hookstyle` (world.c:713) registrations and any remaining reads; `umCtf` from `UserModes_t` (g_local.h:178); "ctf" from `ListGameModes` (commands.c:~8756); `k_ctf_*` block from `resources/example-configs/ktx/ktx.cfg`; the `ctf/` usermode config dir and `id1/maps/ctf/` ent dir from example-configs; `race/` dir from example-configs/ktx.
...
- [ ] `P0` — Remove ToT mode first (Workstream A #5) — it's tangled through the exact bot files povbot touches.
